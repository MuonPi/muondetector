...
void MqttHandler::callback_connected(int result) {
    ...
    std::cout << "Tried to connect " << m_tries << " times. Next try after "
              << (first_retry_period << m_tries) << "s.";
}
...
void MqttHandler::start(const std::string& username, const std::string& password) {
    ...
    CryptoPP::StringSource{
        ...
        new CryptoPP::HashFilter(sha1,
                                 new CryptoPP::HexEncoder(new CryptoPP::StringSink(m_client_id)))};
...
//         return;
//     }
//     result = mosquitto_connect_async(m_mqtt, Config::MQTT::host, Config::MQTT::port,
//     Config::MQTT::keepalive_interval.count()); if (result != MOSQ_ERR_SUCCESS)
//     {
...
//         {
//             std::cerr << "Couldn't publish message for topic" << topic << "(message repeated" <<
//             s_max_publish_errors << "times)";
//         }
//         return;
//     }

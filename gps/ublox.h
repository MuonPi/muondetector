...
public:
  ...
  GnssSatellite(int gnssId, int satId, int cnr, int elev, int azim, float prRes, uint32_t flags) : 
    fGnssId(gnssId), fSatId(satId), fCnr(cnr), fElev(elev), fAzim(azim), fPrRes(prRes), fFlags(flags)
    {}
...
struct gpsProperty {
public:
  gpsProperty() : value(), updated(false) {}  
  gpsProperty(const T& val) : value(val), updated(true) { lastUpdate = std::chrono::system_clock::now(); }  
...
class Ublox
{
   public:
      Ublox();
      Ublox(const std::string& serialPortName);
      Ublox(const std::string& serialPortName, int baudRate);
      ~Ublox();
      ...
      int WriteBuffer(const std::string& buf);
...
//      std::string scanMessage(const std::string& buffer, uint8_t classID, uint8_t messageID);
      ...
      bool waitAck(uint8_t classID, uint8_t messageID, int timeout);
      bool waitAck(uint16_t msgID, int timeout);

//       bool UBXNavClock();
      bool UBXNavClock(int& itow, int& bias, int& drift, int& tAccuracy, int& fAccuracy);
      bool UBXTimTP(int& itow, int& quantErr, int& weekNr);
      bool UBXTimTP();
      bool UBXCfgGNSS();
      bool UBXCfgNav5();
      ...
      bool UBXSetCfgMsg(uint8_t classID, uint8_t messageID, uint8_t port, uint8_t rate);
      bool UBXSetCfgMsg(uint16_t msgID, uint8_t port, uint8_t rate);
      bool UBXSetCfgRate(uint8_t measRate, uint8_t navRate);
	...
	return ts;
      ...
      std::vector<GnssSatellite> UBXNavSat(bool allSats=false);
      
      gpsProperty<int> nrSats;
      gpsProperty<double> timeAccuracy;
      gpsProperty<int> TPQuantErr;
      gpsProperty<double> noise;
      gpsProperty<double> agc;
      gpsProperty<int> txBufUsage;
      gpsProperty<int> txBufPeakUsage;
      gpsProperty<int> leapSeconds;
      gpsProperty<unsigned int> eventCounter;
      gpsProperty<long int> clkBias;
      gpsProperty<long int> clkDrift;

...
Daemon::Daemon(QString username, QString password, QString new_gpsdevname, int new_verbose, quint8 new_pcaPortMask,
               ...
               QString new_daemonAddress, quint16 new_daemonPort, bool new_showout, bool new_showin, bool *preamps,  bool gain, QString station_ID,
               bool* new_polarities, QObject *parent)
    : QTcpServer(parent)
{
    ...
    i2cHandler = new I2cHandler(new_pcaPortMask,new_biasVoltage,new_dacThresh,new_eventTrigger,new_polarities,preamps,bias_ON,gain,verbose);
    connect(i2cHandler, &I2cHandler::setInhibited, pigHandler, &PigpiodHandler::setInhibited);
}
...
Daemon::~Daemon() {
    ...
    snTerm.clear();
    snInt.clear();
    if (i2cHandler!=nullptr){ delete i2cHandler; i2cHandler = nullptr; }
...
void Daemon::connectToPigpiod(){
    ...
    QPointer<TDC7200> tdc7200 = new TDC7200(GPIO_PINMAP[TDC_INTB]);
    ...
    connect(this, &Daemon::setGpioMode, pigHandler, &PigpiodHandler::setGpioMode);
    connect(this, &Daemon::setGpioState, pigHandler, &PigpiodHandler::setGpioState);
    connect(this, &Daemon::gpioRegisterForCallback, pigHandler, &PigpiodHandler::gpioRegisterForCallback);
    ...
    */
    ...
    emit setGpioMode(GPIO_PINMAP[UBIAS_EN], gpio_mode::output);
    emit setGpioState(GPIO_PINMAP[UBIAS_EN], (HW_VERSION==1)?(biasON?1:0):(biasON?0:1));
    emit setGpioMode(GPIO_PINMAP[PREAMP_1], gpio_mode::output);
    emit setGpioMode(GPIO_PINMAP[PREAMP_2], gpio_mode::output);
    emit setGpioMode(GPIO_PINMAP[GAIN_HL], gpio_mode::output);
    emit setGpioState(GPIO_PINMAP[PREAMP_1],preampStatus[0]);
    emit setGpioState(GPIO_PINMAP[PREAMP_2],preampStatus[1]);
    emit setGpioState(GPIO_PINMAP[GAIN_HL],gainSwitch);
    emit setGpioMode(GPIO_PINMAP[STATUS1], gpio_mode::output);
    emit setGpioMode(GPIO_PINMAP[STATUS2], gpio_mode::output);
    emit setGpioState(GPIO_PINMAP[STATUS1], 0);
    emit setGpioState(GPIO_PINMAP[STATUS2], 0);
    emit setGpioMode(GPIO_PINMAP[ADC_READY], gpio_mode::pullup);
    emit gpioRegisterForCallback(GPIO_PINMAP[ADC_READY], 1);

    if (HW_VERSION>1) {
        emit setGpioMode(GPIO_PINMAP[PREAMP_FAULT], gpio_mode::input);
        emit gpioRegisterForCallback(GPIO_PINMAP[PREAMP_FAULT], 0);
        emit setGpioMode(GPIO_PINMAP[PREAMP_FAULT], gpio_mode::pullup);
        emit setGpioMode(GPIO_PINMAP[TDC_INTB], gpio_mode::input);
        emit gpioRegisterForCallback(GPIO_PINMAP[TDC_INTB], 0);
        emit setGpioMode(GPIO_PINMAP[TIME_MEAS_OUT], gpio_mode::input);
        emit gpioRegisterForCallback(GPIO_PINMAP[TIME_MEAS_OUT], 0);
    }
    if (HW_VERSION>=3) {
        emit setGpioMode(GPIO_PINMAP[IN_POL1], gpio_mode::output);
        emit setGpioMode(GPIO_PINMAP[IN_POL2], gpio_mode::output);
        emit setGpioState(GPIO_PINMAP[IN_POL1],polarity1);
        emit setGpioState(GPIO_PINMAP[IN_POL2],polarity2);
    }
}
...
void Daemon::receivedTcpMessage(TcpMessage tcpMessage) {
    ...
    if (msgID == TCP_MSG_KEY::MSG_PREAMP_SWITCH){
        ...
        *(tcpMessage.dStream) >> channel >> status;
        if (channel==0) {
            ...
            emit setGpioState(GPIO_PINMAP[PREAMP_1], status);
            ...
            emit setGpioState(GPIO_PINMAP[PREAMP_2], status);
    ...
    if (msgID == TCP_MSG_KEY::MSG_GAIN_SWITCH){
        ...
        *(tcpMessage.dStream) >> status;
        gainSwitch=status;
        emit setGpioState(GPIO_PINMAP[GAIN_HL], status);
        if (histoMap.find("pulseHeight")!=histoMap.end()) histoMap["pulseHeight"].clear();
        //pulseHeightHisto.clear();
    ...
    if (msgID == TCP_MSG_KEY::MSG_ADC_MODE){
        ...
        *(tcpMessage.dStream) >> mode;
        if (i2cHandler != nullptr){
            i2cHandler->setAdcSamplingMode(mode);
            TcpMessage answer(TCP_MSG_KEY::MSG_ADC_MODE);
            *(answer.dStream) << (quint8)i2cHandler->getAdcSamplingMode();
            emit sendTcpMessage(answer);
        }
    }
    ...
    if (msgID == TCP_MSG_KEY::MSG_GPIO_INHIBIT) {
        ...
        *(tcpMessage.dStream) >> inhibit;
        if (pigHandler!=nullptr) pigHandler->setInhibited(inhibit); // !!! problematic because pigHandler sits in another thread (not threadsafe)
    }
}
    ...
    *(tcpMessage.dStream) << connected;
    if (connected != mqttConnectionStatus) {
        if (connected) {
            ...
            emit setGpioState(GPIO_PINMAP[STATUS1],1);
        }
        else {
...
double Daemon::getRateFromCounts(uint8_t which_rate){
    rateCounterIntervalActualisation();
    QList<quint64> *counts;
    if (which_rate == static_cast<uint8_t>(RATE::XOR_RATE)){
        counts = &xorCounts;
    }else if( which_rate == static_cast<uint8_t>(RATE::AND_RATE)){
        counts = &andCounts;
    }else{
        return -1.0;
    }
...
void Daemon::sendGpioRates(int number, quint8 whichRate){
    ...
    if (whichRate == static_cast<uint8_t>(RATE::XOR_RATE)){
        ratePoints = &xorRatePoints;
    }else if(whichRate == static_cast<uint8_t>(RATE::AND_RATE)){

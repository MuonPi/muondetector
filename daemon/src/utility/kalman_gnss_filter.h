...
class KalmanGnssFilter {
  public:
    ...
    void process(double lat_measurement, double lng_measurement, double alt_measurement,
                 double accuracy);
  ...
  private:
    ...
    double m_accuracy_decay{
        1.}; ///< free parameter describing the decay of accuracy over time, unit: meters per second
    ...
    double m_variance; // P matrix.  Negative means object uninitialised.  NB: units irrelevant, as
                       // long as same units used throughout

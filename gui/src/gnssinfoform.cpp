...
// helper function to format human readable numbers with common suffixes (k(ilo), M(ega), m(illi)
// etc.)
QString printReadableFloat(double value, int prec = 2, int lowOrderInhibit = -12,
                           int highOrderInhibit = 9) {
    QString str = "";
    QString suffix = "";
...
GnssInfoForm::GnssInfoForm(QWidget* parent) : QWidget(parent), ui(new Ui::GnssInfoForm) {
    ui->setupUi(this);
}
...
void GnssInfoForm::onSatsReceived(const QVector<GnssSatellite>& satlist) {
    ...
    for (const auto& current_sat : satlist) {
        ...
        if (current_sat.Health < Gnss::SvHealth::name.size())
            str =
                QString::fromLocal8Bit(Gnss::SvHealth::name[static_cast<int>(current_sat.Health)]);
        ...
        int orbSrc{std::clamp(static_cast<int>(current_sat.OrbitSource),
                              static_cast<int>(Gnss::OrbitSource::first),
        ...
        newItem10->setCheckState((current_sat.Used) ? Qt::CheckState::Checked
        ...
        newItem11->setCheckState((current_sat.DiffCorr) ? Qt::CheckState::Checked
                                                        : Qt::CheckState::Unchecked);
...
void GnssInfoForm::onGpsMonHWReceived(const GnssMonHwStruct& hwstruct) {
    ui->lnaNoiseLabel->setText(QString::number(-hwstruct.noisePerMS) + " dBHz");
    ui->lnaAgcLabel->setText(QString::number(hwstruct.agcCnt));
    QString str{"n/a"};
    if (hwstruct.antStatus < Gnss::AntennaStatus::name.size())
        str =
            QString::fromLocal8Bit(Gnss::AntennaStatus::name[static_cast<int>(hwstruct.antStatus)]);
...
void GnssInfoForm::replotIqWidget() {
    ...
    iqPainter.drawEllipse(QPointF(x, y), fIqData.magI * iqPixmapSize / 512.,
                          fIqData.magQ * iqPixmapSize / 512.);
...
void GnssInfoForm::onGpsVersionReceived(const QString& swString, const QString& hwString,
...
void GnssInfoForm::onGeodeticPosReceived(const GnssPosStruct& pos) {
    QString str{printReadableFloat(pos.hAcc / 1000., 2, 4) + "m / " +
                printReadableFloat(pos.vAcc / 1000., 2, 4) + "m"};

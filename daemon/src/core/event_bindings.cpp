...
void EventBindings::setupDatastore(EventBus& bus, DataStore& datastore) {
    ...
    bus.subscribe<NavClock>(
        [&geoPosManager](const NavClock& event) { geoPosManager.set_time_accuracy(event.tAcc); });
    bus.subscribe<NavStatus>(
        [&geoPosManager](const NavStatus& event) { geoPosManager.set_fix_status(event.gpsFix); });
    ...
    // convert
    bus.subscribe<DatastoreStoreEvent<PreampSwitchEvent>>([&datastore, &bus](const auto& event) {
    ...
    bus.subscribe<MCP4728Event>([&bus](const MCP4728Event& event) {
        bus.publish(ThresholdSettingEvent{
            .channel = 0,
            .voltage = event.voltages.at(MuonPi::Config::Hardware::DAC::Channel::threshold[0])});
        bus.publish(ThresholdSettingEvent{
            .channel = 1,
            .voltage = event.voltages.at(MuonPi::Config::Hardware::DAC::Channel::threshold[1])});
        bus.publish(BiasVoltageEvent{
...
void EventBindings::pollDatastore(EventBus& bus, DataStore& datastore) {

...
namespace MuonPi
{
    ...
    {
        qDebug() << "set_status" << static_cast<int>(status);
        m_status = status;
    }
    ...
    MqttHandler::MqttHandler(const QString &station_id, const int verbosity)
        : QObject(nullptr), m_station_id{station_id.toStdString()}, m_verbose{verbosity}
    {
    ...
    void MqttHandler::onMqttConnect()
    {
        ...
        if (m_mqtt == nullptr)
        {
            qDebug() << "MQTT not initialized on connect called. Initializing...";
            initialise(m_client_id);
        }

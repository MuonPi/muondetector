...
ADS1115::ADS1115()
    : i2cDevice("/dev/i2c-1", 0x49) // NKRG
{
    init();
}

ADS1115::ADS1115(uint8_t slaveAddress) : i2cDevice(slaveAddress) {
    init();
}
...
void ADS1115::waitConversionFinished(bool& error) {
    ...
    int nloops = 0;
    while ((conf_reg & 0x8000) == 0 &&
           nloops * fReadWaitDelay / 1000 < 1000) // readBuf[0] contains 8 MSBs of config register,
                                                  // AND with 10000000 to select bit 15
    {
...
ADS1115::Sample ADS1115::getSample(unsigned int channel) {
    ...
    Sample sample{std::chrono::steady_clock::now(), conv_result,
                  adcToVoltage(conv_result, fPga[fCurrentChannel]),
                  lsb_voltage(fPga[fCurrentChannel]), fCurrentChannel};
...
bool ADS1115::setLowThreshold(int16_t thr) {
    ...
    writeBuf[0] = static_cast<uint8_t>(
        REG::LO_THRESH); // This sets the pointer register to Lo_thresh register
...
bool ADS1115::setHighThreshold(int16_t thr) {
    ...
    writeBuf[0] = static_cast<uint8_t>(
        REG::HI_THRESH); // This sets the pointer register to Hi_thresh register
    writeBuf[1] = (thr & 0xff00) >> 8;
    writeBuf[2] |= (thr & 0x00ff);

...
class PCA9536 : public i2cDevice,
                public DeviceFunction<DeviceType::IO_EXTENDER>,
                public static_device_base<PCA9536> {
  ...
  public:
    ...
    PCA9536() : i2cDevice(0x41) { fTitle = fName = "PCA9536"; }
    PCA9536(const char* busAddress, uint8_t slaveAddress) : i2cDevice(busAddress, slaveAddress) {
        fTitle = fName = "PCA9536";
    }
    PCA9536(uint8_t slaveAddress) : i2cDevice(slaveAddress) { fTitle = fName = "PCA9536"; }
  ...
  private:
    enum REG { INPUT = 0x00, OUTPUT = 0x01, POLARITY = 0x02, CONFIG = 0x03 };
};

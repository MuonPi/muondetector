...
class SHT21 : public i2cDevice {
  public:
    ...
    void setResolutionSettings(
        uint8_t settingsByte); // Sets the resolution Bits for humidity and temperature
    float getTemperature();    // calculates the temperature with the formula in the datasheet. Gets
                               // the solution of read_temp()
    float getHumidity(); // calculates the temperature with the formula in the datasheet. Gets the
                         // solution of read_hum();

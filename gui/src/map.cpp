...
void Map::onGeodeticPosReceived(const GnssPosStruct& pos) {
    ...
    QMetaObject::invokeMethod(
        mapComponent, "setCoordinates", Q_ARG(QVariant, ((double) pos.lon) * 1e-7),
        Q_ARG(QVariant, ((double) pos.lat) * 1e-7), Q_ARG(QVariant, ((double) pos.hMSL) * 1e-3),
        Q_ARG(QVariant, ((double) pos.hAcc) * 1e-3), Q_ARG(QVariant, ((double) pos.vAcc) * 1e-3),
...
void Map::onUiEnabledStateChange(bool connected) {
    ...
    QMetaObject::invokeMethod(mapComponent, "setEnabled", Q_ARG(QVariant, connected));
...
void Map::setConfigPushButtclicked() {
    ...
    posconfig.filter_config =
        static_cast<PositionModeConfig::FilterType>(mapUi->filterTypeComboBox->currentIndex());
    bool ok{false};

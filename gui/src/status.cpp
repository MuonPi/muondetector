...
Status::Status(QWidget* parent) : QWidget(parent), statusUi(new Ui::Status) {
    ...
    statusUi->ratePlotPresetComboBox->addItems(QStringList() << "seconds"
                                                             << "hh:mm:ss"
                                                             << "time");
    statusUi->ratePlotBufferEdit->setText(
        QString("%1:%2:%3:%4")
    ...
    connect(statusUi->ratePlotBufferEdit, &QLineEdit::editingFinished, this,
            [this]() { this->setRateSecondsBuffered(statusUi->ratePlotBufferEdit->text()); });
    connect(statusUi->ratePlotPresetComboBox, &QComboBox::currentTextChanged, statusUi->ratePlot,
            &PlotCustom::setPreset);
    connect(statusUi->resetHistoPushButton, &QPushButton::clicked, this,
            &Status::clearPulseHeightHisto);
    connect(statusUi->resetRatePushButton, &QPushButton::clicked, this, &Status::clearRatePlot);
    connect(statusUi->resetRatePushButton, &QPushButton::clicked, this,
            [this]() { this->resetRateClicked(); });
    connect(statusUi->ratePlotGroupBox, &QGroupBox::toggled, statusUi->ratePlot,
            &PlotCustom::setStatusEnabled);
    connect(statusUi->pulseHeightHistogramGroupBox, &QGroupBox::toggled,
            statusUi->pulseHeightHistogram, &CustomHistogram::setEnabled);
    ...
    connect(statusUi->timingSelectionComboBox, &QComboBox::currentTextChanged, this,
    ...
    if (whichrate == 0) {
        if (xorSamples.size() > 0) {
            for (int k = rates.size() - 1; k >= 0; k--) {
                bool foundK{false}; // if foundK it has found an index k where the x values of the
                                    // input points
                // are smaller or equal to the x values of the already existing points -> overlap
                for (int i = xorSamples.size() - 1; i >= xorSamples.size() - 1 - rates.size();
                     i--) {
                    if (i < 0) {
                        break;
                    }
                ...
                if (foundK && k < rates.size()) {
                    // if there is an overlap, remove all points from input rate points that are
                    // already existing
                    rates.remove(0, k + 1);
                    break;
                }
        ...
        if (andSamples.size() > 0) {
            for (int k = rates.size() - 1; k >= 0; k--) {
                bool foundK = false; // if foundK it has found an index k where the x values of the
                                     // input points
                // are smaller or equal to the x values of the already existing points -> overlap
                for (int i = andSamples.size() - 1; i >= andSamples.size() - 1 - rates.size();
                     i--) {
                    if (i < 0) {
                        break;
                    }
...
void Status::onTriggerSelectionReceived(GPIO_SIGNAL signal) {
    ...
    while (i < statusUi->triggerSelectionComboBox->count()) {
        if (statusUi->triggerSelectionComboBox->itemText(i).compare(
                QString::fromStdString(GPIO_SIGNAL_MAP.at(signal).name)) == 0)
            break;
        i++;
    }
...
void Status::onInputSwitchReceived(TIMING_MUX_SELECTION sel) {
    ...
    while (i < statusUi->timingSelectionComboBox->count()) {
        if (statusUi->timingSelectionComboBox->itemText(i).compare(
                QString::fromStdString(TIMING_MUX_SIGNAL_NAMES.at(sel))) == 0)
            break;
        i++;
    }
...
void Status::onTriggerSelectionComboBoxCurrentIndexChanged(const QString& arg1) {
    for (const auto& [pin, descriptor] : GPIO_SIGNAL_MAP) {
...
void Status::onTimingSelectionComboBoxCurrentIndexChanged(const QString& arg) {
    for (const auto& [mux_signal, mux_name] : TIMING_MUX_SIGNAL_NAMES) {

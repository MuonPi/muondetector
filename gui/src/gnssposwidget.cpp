...
constexpr double sqrt2() {
    return Detail::sqrt(2.);
}

constexpr int MAX_SAT_TRACK_ENTRIES{1000};

static const QVector<QColor> GNSS_COLORS = {Qt::darkGreen, Qt::darkYellow, Qt::blue, Qt::magenta,
                                            Qt::gray,      Qt::cyan,       Qt::red,  Qt::black};
...
GnssPosWidget::GnssPosWidget(QWidget* parent) : QWidget(parent), ui(new Ui::GnssPosWidget) {
    ui->setupUi(this);
    ui->satLabel->setText(QString::number(ui->satLabel->width()) + "x" +
                          QString::number(ui->satLabel->height()));
    ...
    connect(ui->cartPolarCheckBox, &QCheckBox::toggled, this,
...
QPolygonF GnssPosWidget::getCartPolygonUnity(const QPointF& polarPos) {
    QPolygonF polarRect{
        getPolarUnitPolygon(polarPos, DEFAULT_CONTROL_POINTS + qAbs(polarPos.y() / 10))};
...
void GnssPosWidget::drawPolarPixMap(QPixmap& pm) {
    ...
    satPosPainter.drawEllipse(QPoint(satPosPixmapSize / 2, satPosPixmapSize / 2),
                              satPosPixmapSize / 6, satPosPixmapSize / 6);
    satPosPainter.drawEllipse(QPoint(satPosPixmapSize / 2, satPosPixmapSize / 2),
                              satPosPixmapSize / 3, satPosPixmapSize / 3);
    satPosPainter.drawEllipse(QPoint(satPosPixmapSize / 2, satPosPixmapSize / 2),
                              satPosPixmapSize / 2, satPosPixmapSize / 2);
    satPosPainter.drawLine(QPoint(satPosPixmapSize / 2, 0),
                           QPoint(satPosPixmapSize / 2, satPosPixmapSize));
    satPosPainter.drawLine(QPoint(0, satPosPixmapSize / 2),
                           QPoint(satPosPixmapSize, satPosPixmapSize / 2));
    satPosPainter.drawText(satPosPixmapSize / 2 + 2, 3, 18, 18, Qt::AlignHCenter, "N");
    satPosPainter.drawText(satPosPixmapSize / 2 + 2, satPosPixmapSize - 19, 18, 18,
                           Qt::AlignHCenter, "S");
    satPosPainter.drawText(4, satPosPixmapSize / 2 - 19, 18, 18, Qt::AlignHCenter, "W");
    satPosPainter.drawText(satPosPixmapSize - 19, satPosPixmapSize / 2 - 19, 18, 18,
                           Qt::AlignHCenter, "E");

    QFont font = satPosPainter.font();
    font.setPointSize(font.pointSize() - 2);
    satPosPainter.setFont(font);
    satPosPainter.drawText(satPosPixmapSize / 2 - 14, satPosPixmapSize - 12, 18, 18,
                           Qt::AlignHCenter, "0°");
    satPosPainter.drawText(satPosPixmapSize / 2 - 16, satPosPixmapSize * 5 / 6 - 12, 18, 18,
                           Qt::AlignHCenter, "30°");
    satPosPainter.drawText(satPosPixmapSize / 2 - 16, satPosPixmapSize * 2 / 3 - 12, 18, 18,
                           Qt::AlignHCenter, "60°");
        ...
        for (const auto& pointMap : satTracks) {
            for (const auto& pointVector : pointMap) {
                if (pointVector.isEmpty())
                    continue;
                const double mean_cnr{std::accumulate(pointVector.constBegin(),
                                                      pointVector.constEnd(), 0,
                                                      [](int sum, const SatHistoryPoint& satpoint) {
                                                          return sum + satpoint.cnr;
                                                      }) /
                                      static_cast<double>(pointVector.size())};
                const auto satPoint{pointVector.front()};
                const int id = static_cast<int>(satPoint.gnssId);
                const int maxIndex = static_cast<int>(GNSS_COLORS.size()) - 1;
                const int clampedId = std::clamp(id, 0, maxIndex);
                QColor satColor{GNSS_COLORS.at(clampedId)};
    ...
    for (const auto& currentSat : fCurrentSatlist) {
        ...
        if (currentSat.Elev <= 90. && currentSat.Elev >= -90.) {
            ...
            const int id = static_cast<int>(currentSat.GnssId);
            const int maxIndex = static_cast<int>(GNSS_COLORS.size()) - 1;
            const int clampedId = std::clamp(id, 0, maxIndex);
            QColor satColor{GNSS_COLORS.at(clampedId)};
            ...
            // somehow, drawEllipse doesn't work with a painter when a transform is set before
            // (bug?) so reset the transform of the painter and do the scaling and shifting manually
...
void GnssPosWidget::drawCartesianPixMap(QPixmap& pm) {
    ...
    satPosPainter.drawText(3 * pm.width() / 4 - 3, originOffset.y() + 6, 18, 18, Qt::AlignHCenter,
                           "W");
    ...
    satPosPainter.drawText(pm.width() / 2, 0.33 * originOffset.y() + 3, 18, 18, Qt::AlignHCenter,
                           "60°");
    satPosPainter.drawText(pm.width() / 2, 0.67 * originOffset.y() + 3, 18, 18, Qt::AlignHCenter,
                           "30°");
    ...
    for (const auto& currentSat : fCurrentSatlist) {
        ...
        if (currentSat.Elev <= 90. && currentSat.Elev >= -90.) {
            ...
            currPos = QPointF(currPos.x() / 360. * pm.width(),
                              -currPos.y() / 90. * pm.height() * 0.9 + pm.height() * 0.9);
...
void GnssPosWidget::exportToFile() {
    ...
    QString fn =
        QFileDialog::getSaveFileName(this, tr("Export Pixmap"), suggestedName, types, &filter);
    ...
    if (!fn.isEmpty()) {            // If filename is not null
        ...
        if (filter.contains(jpegExt)) { // OR, Test to see if jpeg and save
            ...
            out << "# <ISO8601-datetime> <gnss-id> <gnss-id-string> <sat-id> <azimuth> <elevation> "
                   "<cnr>\n";
            ...
            for (const auto& pointMap : satTracks) {
                for (const auto& pointVector : pointMap) {
                    ...
                    const double mean_cnr{
                        std::accumulate(pointVector.constBegin(), pointVector.constEnd(), 0,
                                        [](int sum, const SatHistoryPoint& satpoint) {
                                            return sum + satpoint.cnr;
                                        }) /
                        static_cast<double>(pointVector.size())};
                    ...
                    out << p.time.toString(Qt::ISODate) << " " << p.gnssId << " "
                        << QString::fromLocal8Bit(Gnss::Id::name[std::clamp(
                               static_cast<int>(p.gnssId), static_cast<int>(Gnss::Id::first),
                               static_cast<int>(Gnss::Id::last))])
                        << p.satId << " " << p.posPolar.x() << " " << p.posPolar.y() << " "
                        << mean_cnr << "\n";
                }
            }
        }

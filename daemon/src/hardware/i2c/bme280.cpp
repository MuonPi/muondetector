...
bool BME280::setMode(uint8_t mode) { // 0= skipped; oversampling: 1: x1; 2:x2; 3: x4; 4:x8; 5:x16
    if (mode > 0b11) {
        if (fDebugLevel > 1)
            printf("mode > 3 error\n");
        return false;
...
bool BME280::setTSamplingMode(
    uint8_t mode) { // 0= skipped; oversampling: 1: x1; 2:x2; 3: x4; 4:x8; 5:x16
    if (mode > 0b111) {
        if (fDebugLevel > 1)
            printf("mode > 3 error\n");
        return false;
...
bool BME280::setDefaultSettings() {
    ...
    write_CtrlMeasReg(0b01001000); // enabling temperature and pressure measurement (oversampling
                                   // x2), set sleep mode
    return true;
}
...
bool BME280::measure() {
    ...
    setMode(
        0x2); // set mode to "forced measurement" (single-shot)
...
int32_t BME280::readUT() {
    ...
    val = ((uint32_t) readBuf[0]) << 12; // <-------///// should the lower 4 bits or the higher 4
                                         // bits of the 24 bits of registers be left 0 ???
...
double BME280::getHumidity(
    int32_t adc_H) { // please only do this if "getTemperature()" has been executed before
...
double BME280::getHumidity(int32_t adc_H, int32_t t_fine) {
    ...
    X1 = (((((adc_H << 14) - (dig_H4 << 20) - (dig_H5 * X1)) + ((int32_t) 16384)) >> 15) *
          (((((((X1 * dig_H6) >> 10) * (((X1 * dig_H3) >> 11) + ((int32_t) 32768))) >> 10) +
             ((int32_t) 2097152)) *
                dig_H2 +
            8192) >>
           14));

...
void GeoPosManager::set_histos(std::shared_ptr<Histogram> lon, std::shared_ptr<Histogram> lat,
...
void GeoPosManager::new_position(const GeoPosition& new_pos) {
    ...
    switch (m_mode_config.filter_config) {
        case PositionModeConfig::FilterType::None:
            ...
            break;
        case PositionModeConfig::FilterType::Kalman:
            m_gnss_pos_kalman.process(new_pos.latitude, new_pos.longitude, new_pos.altitude,
                                      totalPosAccuracy);
            preliminary_pos = {m_gnss_pos_kalman.get_longitude(), m_gnss_pos_kalman.get_latitude(),
...
GeoPosition GeoPosManager::get_pos_from_histos() const {
    ...
    const double degree_to_surface_meters{(pi() * 2 * earth_radius_meters) / 360.};
    ...
    preliminary_pos.hor_error += sqr(m_lon_histo->getRMS() * degree_to_surface_meters / pi() *
                                     preliminary_pos.latitude / 180.);
...
void GeoPosManager::check_for_lockin_reached(const GeoPosition& preliminary_pos) {
    ...
    if (preliminary_pos.vert_error < m_mode_config.lock_in_min_error_meters &&
        ...
        sstr << "concluded geo pos lock-in and fixed position: lat=" << preliminary_pos.latitude
             << "lon=" << preliminary_pos.longitude << "(err=" << preliminary_pos.hor_error << "m)"
             << "alt=" << preliminary_pos.altitude << "(err=" << preliminary_pos.vert_error << "m)";
        logInfo(sstr.str());
    }
}

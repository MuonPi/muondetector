...
CalibScanDialog::CalibScanDialog(QWidget* parent) : QDialog(parent), ui(new Ui::CalibScanDialog) {
    ui->setupUi(this);
    connect(ui->startCurrentCalibPushButton, &QPushButton::clicked, this,
            &CalibScanDialog::startManualCurrentCalib);
    connect(ui->transferCurrentBiasPushButton, &QPushButton::clicked, this,
            &CalibScanDialog::transferCurrentCalibCoeffs);
...
void CalibScanDialog::manualCurrentCalibProgress(double vbias, double ibias) {
    ...
    if (fCurrentCalibRunning == 2) {
        ...
        msgBox.setInformativeText(
            "Connect the cable again to the bias connector of the MuonPi PCB!");
        msgBox.setStandardButtons(QMessageBox::Ok | QMessageBox::Cancel);
        msgBox.setDefaultButton(QMessageBox::Ok);
        int ret = msgBox.exec();
        if (ret == QMessageBox::Cancel) {
...
void CalibScanDialog::transferCurrentCalibCoeffs() {
    ...
    emit dynamic_cast<CalibForm*>(parent())->setCalibParameter("CALIB_FLAGS",

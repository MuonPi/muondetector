...
ParameterMonitorForm::ParameterMonitorForm(QWidget* parent)
    ...
    connect(ui->preamp2EnCheckBox, &QCheckBox::clicked, this,
            &ParameterMonitorForm::preamp2EnableChanged);
    connect(ui->biasEnCheckBox, &QCheckBox::clicked, this,
            &ParameterMonitorForm::biasEnableChanged);
    connect(ui->hiGainCheckBox, &QCheckBox::clicked, this,
            &ParameterMonitorForm::gainSwitchChanged);
    connect(ui->pol1CheckBox, &QCheckBox::clicked, this,
            &ParameterMonitorForm::onPolarityCheckBoxClicked);
    connect(ui->pol2CheckBox, &QCheckBox::clicked, this,
            &ParameterMonitorForm::onPolarityCheckBoxClicked);
    connect(ui->timingSelectionComboBox, &QComboBox::currentIndexChanged, this,
            &ParameterMonitorForm::onTimingSelectionComboBoxCurrentIndexChanged);
    connect(ui->adcTriggerSelectionComboBox, &QComboBox::currentIndexChanged, this,
            &ParameterMonitorForm::onAdcTriggerSelectionComboBoxCurrentIndexChanged);
}

void ParameterMonitorForm::onTimingSelectionComboBoxCurrentIndexChanged(int index) {
...
void ParameterMonitorForm::onAdcTriggerSelectionComboBoxCurrentIndexChanged(int index) {
    for (const auto& [pin, descriptor] : GPIO_SIGNAL_MAP) {
        if (QString::fromStdString(descriptor.name) ==
            ui->adcTriggerSelectionComboBox->itemText(index)) {
...
void ParameterMonitorForm::onInputSwitchReceived(TIMING_MUX_SELECTION sel) {
    ...
    while (i < ui->timingSelectionComboBox->count()) {
        if (ui->timingSelectionComboBox->itemText(i).compare(
...
void ParameterMonitorForm::onTriggerSelectionReceived(GPIO_SIGNAL signal) {
    ...
    while (i < ui->adcTriggerSelectionComboBox->count()) {
        if (ui->adcTriggerSelectionComboBox->itemText(i).compare(
...
void ParameterMonitorForm::onDaemonVersionReceived(MuonPi::Version::Version hw_ver,

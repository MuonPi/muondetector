...
EEPROM24AA02Driver::EEPROM24AA02Driver(ComponentId id, DeviceRegistry& registry, EventBus& bus)
    ...
    if (wrapper) {
        calib = std::make_shared<ShowerDetectorCalib>(&wrapper->device());

        if (calib->isEepromValid()) {
            ...
            logInfo("Found HW version " + std::to_string(MuonPi::Version::hardware.major) +
                    " in eeprom");
        }
    } else {
        logWarn("EEPROM not available, using default calibration");

        calib = std::make_shared<ShowerDetectorCalib>();
    }

...
class EEPROM24AA02 : public i2cDevice,
                     public DeviceFunction<DeviceType::EEPROM>,
                     public static_device_base<EEPROM24AA02> {
  public:
    EEPROM24AA02() : i2cDevice(0x50) { fTitle = fName = "24AA02"; }
    EEPROM24AA02(const char* busAddress, uint8_t slaveAddress)
        : i2cDevice(busAddress, slaveAddress) {
        fTitle = fName = "24AA02";
    }
    EEPROM24AA02(uint8_t slaveAddress) : i2cDevice(slaveAddress) { fTitle = fName = "24AA02"; }
  ...
  private:
    // hide all write functions from i2cDevice class since they do not conform to the correct write
    // sequence of the eeprom

...
uint8_t
SHT21::readResolutionSettings() { // reads the temperature and humidity resolution settings byte
    uint8_t readBuf[1];
...
void SHT21::setResolutionSettings(
    uint8_t settingsByte) { // sets the temperature and humidity resolution settings byte
    uint8_t readBuf[1];
    readBuf[0] = 0; // Initialization
    readReg(0xE7, readBuf, 1);

...
bool MCP4728::writeChannel(uint8_t channel, const DacChannel& channelData) {
    ...
    buf[0] |=
        (channel
         << 1); // 01000/01011 (multiwrite/singlewrite command) DAC1 DAC0 (channel) UDAC bit = 1
...
bool MCP4728::storeSettings() {
    ...
    for (uint8_t channel = 0; channel < 4; channel++) {
        buf[channel * 2 + 1] = ((uint8_t) fChannelSetting[channel].vref)
                               << 7; // Vref PD1 PD0 Gx (gain) D11 D10 D9 D8
        ...
        buf[channel * 2 + 2] =
            (uint8_t) (fChannelSetting[channel].value & 0xff); // D7 D6 D5 D4 D3 D2 D1 D0
    }
...
bool MCP4728::readRegisters() {
    ...
    if ((std::chrono::steady_clock::now() - fLastRegisterUpdate) < DataValidityTimeout) {
        //		std::cout<<std::dec<<"debug: data
        // age="<<(std::chrono::duration_cast<std::chrono::milliseconds>(
        // std::chrono::steady_clock::now()-fLastRegisterUpdate ).count())<<"ms"<<std::endl;
        return true;
    }
...
bool MCP4728::identify() {
    ...
    if (((buf[0] & 0xf0) == 0xc0) && ((buf[6] & 0xf0) == 0xd0) && ((buf[12] & 0xf0) == 0xe0) &&
        ((buf[18] & 0xf0) == 0xf0)) {
        return true;
    }
...
bool MCP4728::setVRef(CFG_VREF vref_setting) {
    ...
    fChannelSetting[0].vref = fChannelSetting[1].vref = fChannelSetting[2].vref =
        fChannelSetting[3].vref = vref_setting;
    return true;
}
...
void MCP4728::dumpRegisters() {
    ...
    for (int ch = 0; ch < 4; ch++) {
        std::cout << "DAC" << ch << ": " << std::setw(2) << std::setfill('0') << std::hex
                  << (int) buf[ch * 6] << " " << (int) buf[ch * 6 + 1] << " "
                  << (int) buf[ch * 6 + 2] << " (eep: " << (int) buf[ch * 6 + 3] << " "
                  << (int) buf[ch * 6 + 4] << " " << (int) buf[ch * 6 + 5] << ")\n";
    }
}

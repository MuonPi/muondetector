...
static const std::string base32 =
    "0123456789bcdefghjkmnpqrstuvwxyz"; // (geohash-specific) Base32 map

/**
 * Geohash: Gustavo Niemeyer’s geocoding system.
 */
auto GeoHash::hashFromCoordinates(double lon, double lat, int precision) -> std::string {
    if (precision < 0 || precision > 12)
        precision = 12;

    std::uint8_t idx = 0; // index into base32 map
    std::uint8_t bit = 0; // each char holds 5 bits
    bool evenBit = true;
    std::string geohash = "";

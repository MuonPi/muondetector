...
int main(int argc, char* argv[]) {
    ...
    LM75 lm75{"/dev/i2c-1", address};
    if (!lm75.identify()) {
        std::cerr << "LM75 not found at address 0x" << std::hex << static_cast<int>(address)
                  << std::dec << "\n";
        return 1;
    }
    ...
    while (true) {
        std::cout << std::fixed << std::setprecision(2) << lm75.getTemperature() << "\n";
        std::cout << std::flush;

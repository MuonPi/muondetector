...
MCP4728Driver::MCP4728Driver(ComponentId id, SystemConfig& systemConfig, DeviceRegistry &registry, EventBus &bus)
    : Component(id), registry_{registry}, bus_{bus}
{
    ...
    if (!device || !device->probeDevicePresence())
    {
        ...
        for (std::uint8_t channel = 0; channel < 4; channel++) {
            ...
            sstr << " ch" << static_cast<unsigned>(channel) << ": " << dacChannel.value << "=" << MCP4728::code2voltage(dacChannel) << "V"
        ...
        for (std::uint8_t channel = 0; channel < 2; channel++) {
            if (systemConfig.thresholdVoltage[Config::Hardware::DAC::Channel::threshold[channel]] < 0.) {
                systemConfig.thresholdVoltage[Config::Hardware::DAC::Channel::threshold[channel]] = MCP4728::code2voltage(data.at(channel));
            }
        }
        ...
        if (eepromBias.value == 0) {
            ...
            device->setVoltage(Config::Hardware::DAC::Channel::bias, Config::Hardware::DAC::Voltage::bias);
            device->setVoltage(Config::Hardware::DAC::Channel::threshold[0], Config::Hardware::DAC::Voltage::threshold[0]);
            device->setVoltage(Config::Hardware::DAC::Channel::threshold[1], Config::Hardware::DAC::Voltage::threshold[1]);
        }

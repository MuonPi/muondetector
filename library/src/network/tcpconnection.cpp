...
TcpConnection::TcpConnection(tcp::socket socket)
    : socket_(std::move(socket)), lastReceivedNanos_(nowNanos()) {
...
void TcpConnection::sendPacket(std::uint16_t key, std::vector<std::uint8_t>&& payload) {
    ...
    boost::asio::post(socket_.get_executor(),
                      [this, self, key, payload = std::move(payload)]() mutable {
                          bool writing = !writeQueue_.empty();
                          writeQueue_.push_back(buildFrame(key, payload));
                          if (!writing) {
                              do_write();
...
auto TcpConnection::tryParsePacket() -> bool {
    ...
    if (version != kProtocolVersion) {
        closeWithError(
            boost::asio::error::make_error_code(boost::asio::error::operation_not_supported));
        return false;
    }
...
auto TcpConnection::buildFrame(std::uint16_t key, const std::vector<std::uint8_t>& payload)
...
auto TcpConnection::nowNanos() -> std::uint64_t {
    return static_cast<std::uint64_t>(
        std::chrono::duration_cast<std::chrono::nanoseconds>(clock_type::now().time_since_epoch())
            .count());
}

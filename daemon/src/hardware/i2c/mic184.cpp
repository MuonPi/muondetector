 ...
 */
MIC184::MIC184() : i2cDevice(0x4f) {
    fTitle = fName = "MIC184";
}

MIC184::MIC184(const char* busAddress, uint8_t slaveAddress) : i2cDevice(busAddress, slaveAddress) {
    fTitle = fName = "MIC184";
}

MIC184::MIC184(uint8_t slaveAddress) : i2cDevice(slaveAddress) {
    fTitle = fName = "MIC184";
}
...
bool MIC184::identify() {
    ...
    if ((thyst_save & 0x7f) != 0 || (tos_save & 0x7f) != 0) {
        return false;
    }

    if (fDebugLevel > 0) {
        std::cout << "MIC184::identify() : found LM75 based device at 0x" << std::setw(2)
                  << std::setfill('0') << std::hex << (int) fAddress << "\n";
        std::cout << " Regs: \n";
    ...
    // datasheet: test, if the STS (status) bit in config register toggles when an alarm condition
    // is provoked set config reg to 0x02

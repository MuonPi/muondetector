...
void KalmanGnssFilter::process(double lat_measurement, double lng_measurement,
                               double alt_measurement, double accuracy) {
    ...
    if (m_variance < 0) {
        ...
        unsigned int time_increment_ms =
            std::chrono::duration_cast<std::chrono::milliseconds>(current_time - m_timestamp)
                .count();
        ...
        // NB: because K is dimensionless, it doesn't matter that variance has different units to
        // lat and lng

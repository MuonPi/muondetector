...
void LogPlotsWidget::updateLogTable() {
    ...
    for (auto it = fLogMap.begin(); it != fLogMap.end(); it++) {
        ...
        QTableWidgetItem* newItem2 =
            new QTableWidgetItem(QString::number(it.value().data().size()));
...
void LogPlotsWidget::onScalingChanged() {
    ui->xMinLineEdit->setText(
        QString::number(ui->logPlot->axisInterval(QwtPlot::xBottom).minValue()));
    ui->xMaxLineEdit->setText(
        QString::number(ui->logPlot->axisInterval(QwtPlot::xBottom).maxValue()));
    ui->yMinLineEdit->setText(
        QString::number(ui->logPlot->axisInterval(QwtPlot::yLeft).minValue()));
...
void LogPlotsWidget::onLogInfoReceived(const LogInfoStruct& lis) {
    ui->dataFileNameLineEdit->setText(QString::fromStdString(lis.dataFileName));
    ui->logFileNameLineEdit->setText(QString::fromStdString(lis.logFileName));

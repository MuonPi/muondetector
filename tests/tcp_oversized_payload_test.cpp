...
namespace {
// Framing helpers for crafting oversize header.
...
int main() {
    ...
    // Payload size > protocol maximum should be rejected from header alone.
    std::vector<std::uint8_t> frame;
    const auto m = be32(0x4D554F4Eu);
    const auto v = be16(1);
    const auto k = be16(2);
    const auto s = be32(static_cast<std::uint32_t>(TcpConnection::kMaxPayloadSize + 1));
    ...
    // no payload/trailer needed; parser rejects as soon as header is complete
    ...
    if (!waitForConnectionCount(*sink, 0, std::chrono::seconds(2))) {
        std::cerr << "oversized payload did not trigger cleanup\n";
        io->stop();
        ioThread.join();
        return 1;
    }

...
class DeviceFunctionBase {
  public:
     ...
     * @note The probing should be non-invasive, i.e. most devices will not change settings when a
     * read access is performed. This method must be implemented in the derived classes.
     * */
...
/// all methods which are specific for a device function shall be declared in the template
...
/** specialization for ADCs
 the specialization has getVoltage(), getSample() and triggerConversion() methods, which must be
...
class DeviceFunction<DeviceType::ADC> : public DeviceFunctionBase<DeviceType::ADC> {
  public:
    ...
    static constexpr Sample InvalidSample{std::chrono::steady_clock::time_point::min(), 0, 0., 0.,
                                          0};
...
/** specialization for EEPROMs
 the specialization has readBytes() and writeBytes methods, which must be implemented in the derived
 class
*/
template <>
...
/** specialization for I/O extenders
 the specialization has setOutputPorts(), setOutputState and getInputState methods, which must be
 implemented in the derived class
*/
template <>

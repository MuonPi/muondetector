...
const std::unordered_map<ComponentId, DeviceComponentCreator> ComponentFactory::deviceComponentCreator = {
    {Device::MCP4728_0,
     [](Context &ctx) {
         return std::make_shared<MCP4728Driver>(Device::MCP4728_0, *ctx.config, *ctx.registry, *ctx.bus);
     }},
...
const std::unordered_map<ComponentId, ComponentCreator> ComponentFactory::componentCreator = {
    {OtherComponent::TCP_SOURCE_0,
     [](Context &ctx) { return std::make_shared<TcpSource>(OtherComponent::TCP_SOURCE_0, *ctx.bus); }},
    {OtherComponent::GPS_DRIVER_0, [](Context &ctx) {
         return std::make_shared<SerialUblox>(OtherComponent::GPS_DRIVER_0, *ctx.io, ctx.config->gpsdevname,
                                              ctx.config->gnss_baudrate, *ctx.bus);
     }}};
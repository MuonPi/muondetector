...
void i2cDevice::setAddress(uint8_t address) { // pointer to our device on the i2c-bus
    fAddress = address;
    int res = ioctl(fHandle, I2C_SLAVE,
                    fAddress); // i.e. specify the address of the I2C slave to communicate with
    if (res < 0) {
...
bool i2cDevice::identify() {
    ...
    // if it is not reimplemented it returns false, i.e. the abstract i2cDevice can never be
    // identified positively reimplement this method for specific devices and use whatever is needed
    // to get an unanimous identification return true if device is identified correctly
    return false;
}

int i2cDevice::read(
    uint8_t* buf,
    int nBytes) { // defines a function with a pointer buf as buffer and the number of bytes which
    // we want to read.
    if (fHandle <= 0 || (fMode & MODE_LOCKED))
        return 0;
    // std::lock_guard<std::mutex> lock(fMutex);
    int nread = ::read(fHandle, buf, nBytes); //"::" declares that the functions does not call
                                              // itself again, but instead uses the system call
    if (nread > 0) {
        fNrBytesRead += nread;
...
bool i2cDevice::writeWords(uint8_t regAddr, uint16_t length, uint16_t* data) {
    ...
    while ((nwritten = writeReg(regAddr, buf, length * 2)) != length * 2 &&
           retry_ctr++ < fNrRetries) {
    };

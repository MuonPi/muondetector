...
int main()
{
    ...
    // Header with wrong magic must be rejected immediately.
    const auto m = be32(0x12345678u); // bad magic
    const auto v = be16(1);
    const auto k = be16(2);
    const auto s = be32(0);
    const auto c = be32(2166136261u); // checksum(empty)
    ...
    if (!waitForConnectionCount(*sink, 0, std::chrono::seconds(2))) {
        std::cerr << "bad magic did not trigger cleanup\n";
        io->stop();
        ioThread.join();
        return 1;
    }

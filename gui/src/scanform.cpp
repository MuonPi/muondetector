    ...
    if (active && OP_NAMES[obsPar] == "UBXRATE") {
        if (waitForFirst) {
            ...
            if ((currentTimeInterval > maxMeasurementTimeInterval) ||
                ((currentCounts > maxMeasurementStatistics) && (currentTimeInterval > 1))) {
                scanParIteration();
            }
        }
    }
...
void ScanForm::scanParIteration() {
    ...
    if (OP_NAMES[obsPar] == "UBXRATE") {
        ...
        ui->currentObservableLabel->setText(QString::number(rate) + " +- " +
                                            QString::number(scanData[currentScanPar].error));
        updateScanPlot();
    }
    ...
    if (!fn.isEmpty()) { // If filename is not null
        ...
        if (filter.contains(txtExt)) {
            ...
            for (auto it = scanData.constBegin(); it != scanData.constEnd(); ++it) {
                ...
                out << QString::number(x, 'g', 10) << "  " << QString::number(point1.value, 'g', 10)
